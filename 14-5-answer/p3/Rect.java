...
public class Rect extends Shape{
	...
	public Rect(int x1, int y1, int x2, int y2, Color color, Color color2, boolean filled, boolean gradient) {
		super(x1, y1, x2, y2, color, color2, filled, gradient);
		// TODO Auto-generated constructor stub
	}
	...
	public void draw(Graphics g) {
		if(gradient){
			System.out.println("gradient=true");
			...
			System.out.println("gradient=false");
			Graphics2D g2d = (Graphics2D) g;
			g2d.setPaint(color);
			if(filled){
				g2d.fillRect(Math.min(x1,x2), Math.min(y1, y2), Math.abs(x1-x2), Math.abs(y1-y2));
